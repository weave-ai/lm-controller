...
IMG ?= ghcr.io/weave-ai/lm-controller:latest
# ENVTEST_K8S_VERSION refers to the version of kubebuilder assets to be downloaded by envtest binary.
ENVTEST_K8S_VERSION = 1.27.1

# Use the same setting as the source-controller
...
ifeq ($(TARGET_TEST),)
	...
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test ./... -run $(TARGET_TEST) -coverprofile cover.out
endif
...
build: manifests generate fmt vet ## Build manager binary.
	go build -o $(BUILD_DIR)/bin/manager cmd/main.go
